 ...
 where TSpan : IMerge<TSpan>, new()
    {
      ...
      /// <param name="size"></param>
        ...
        private void ReportError()
        {
            ...
            List<int> expectedTerminals = this.FsaState.ParserTable.Keys
                .Where(t => t != this.endOfFileToken && t != this.errorToken)
                .ToList();

            if (expectedTerminals.Count < 7)
            {
                for (int i = 0; i < expectedTerminals.Count; i++)
                {
                    if (i > 0)
                    {
                        errorMessage.Append(i == expectedTerminals.Count - 1 ? " or " : ", ");
                    }

                    errorMessage.Append(this.TerminalToString(expectedTerminals[i]));
                }
            }

            errorMessage.Append(" expected");

            this.scanner.yyerror(errorMessage.ToString());
        }
